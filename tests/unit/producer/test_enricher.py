...
mock_cache_response = {'Q123': {'label': 'Cache Label 123', 'description': 'Cache Desc 123'}}
...
@pytest.mark.parametrize("test_id, events, cache_return, api_response, api_error, expected_events", [
    pytest.param("cache_and_api", events_mixed, mock_cache_response, mock_api_response_json, None, expected_mixed, id="cache_and_api"),
...
def test_enrich_scenarios(mocker, enricher, test_id, events, cache_return, api_response, api_error, expected_events):
    ...
    qids_in_batch = {e['title'] for e in events if e['title'].startswith('Q')}
    ...
    if api_error or not qids_to_fetch:
