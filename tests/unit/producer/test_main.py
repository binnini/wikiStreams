...
def test_run_producer_initializes_and_runs_pipeline(mock_dependencies):
    ...
    mock_dependencies["Sender"].assert_called_once_with(main.KAFKA_BROKER, main.KAFKA_TOPIC)
    mock_dependencies["Collector"].assert_called_once_with(main.BATCH_SIZE, main.BATCH_TIMEOUT_SECONDS)
    
    # 3. 콜백 함수가 올바르게 설정되었는지 확인
    mock_dependencies["collector_instance"].set_callback.assert_called_once_with(ANY) # process_batch 함수
...
def test_process_batch_callback_flow(mock_dependencies):
    ...
    process_batch_callback = mock_dependencies["collector_instance"].set_callback.call_args[0][0]
    ...
    mock_dependencies["enricher_instance"].enrich_events.assert_called_once_with(test_events)
    mock_dependencies["sender_instance"].send_events.assert_called_once_with(enriched_events)
