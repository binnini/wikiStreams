...
@pytest.mark.parametrize(
    "test_id, sse_events, time_side_effect, expected_callback_arg, reconnect_side_effect",
    [
        pytest.param(
            "by_batch_size",
            sse_batch_size,
            None,
            [event_data_1, event_data_2],
            None,
            id="by_batch_size",
        ),
        pytest.param(
        ...
        pytest.param(
            ...
            sse_json_error,
            None,
            [event_data_valid_1, event_data_valid_2],
            None,
            id="with_json_error",
        ),
        pytest.param(
...
def test_collector_scenarios(
    mocker,
    collector,
    test_id,
    sse_events,
    time_side_effect,
    expected_callback_arg,
    reconnect_side_effect,
):
    # --- Arrange ---
        ...
        success_stream = MagicMock(
            __enter__=MagicMock(
                return_value=MagicMock(
                    iter_sse=MagicMock(return_value=stream_generator(sse_events))
                )
            )
        )
