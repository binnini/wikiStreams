...
[![Python Code Quality CI](https://github.com/<Your-GitHub-Username>/<Your-Repository-Name>/actions/workflows/ci.yml/badge.svg)](https://github.com/<Your-GitHub-Username>/<Your-Repository-Name>/actions/workflows/ci.yml)

**WikiStreams**는 전 세계 위키미디어(위키피디아 등)의 실시간 변경 로그 스트림을 분석하여 트렌드를 파악하는 데이터 파이프라인 프로젝트입니다. M4 Mac Mini를 활용한 홈 랩(Home Lab) 환경에서 운영되며, 비용을 들이지 않고 현업 수준의 실시간 데이터 처리 아키텍처를 구축하는 것을 목표로 합니다.
...
graph TD
    ...
    subgraph "M4 Mac Mini (Docker Host)"
        ...
        subgraph "Ingestion"
            B(<fa:fa-brands fa-python> Python Producer)
        end

        subgraph "Message Bus"
            C{<fa:fa-layer-group> Apache Kafka}
        end
        ...
        subgraph "Processing & Storage"
            ...
            subgraph "⚡ Hot Path"
                D[<fa:fa-bolt> Apache Druid]
            end
            
            subgraph "🧊 Cold Path (Future)"
                E[<fa:fa-archive> Kafka Connect] --> F[<fa:fa-database> S3 Bucket]
            end
        end

        subgraph "Serving (Future)"
    ...
    B -- "2. Raw JSON" --> C
    C -- "3a. Stream for Analytics" --> D
    C -- "3b. Stream for Backup" --> E
    D -- "4. Aggregated Data" --> G
...
*   **Ingestion:** `httpx`를 사용한 Python Producer가 이벤트를 Kafka로 전송
*   **Message Bus:** Apache Kafka (KRaft 모드, Zookeeper-less)가 데이터 허브 역할 수행
*   **Real-time Analytics (Hot Path):** Apache Druid가 Kafka로부터 데이터를 실시간으로 수집하고 집계
*   **Data Lake (Cold Path):** (향후 도입) Kafka Connect를 통해 S3에 원본 데이터 백업
*   **Visualization (Serving):** (향후 도입) Apache Superset을 통해 Druid 데이터 시각화
