...
- [x] **일일 트렌드 뉴스 리포트 (Discord Embed, 한국어)** *(2026-03-01 완료)*
  ...
  - **데이터 수집** (`src/reporter/fetcher.py`): 전체 통계, 상위 편집 문서(봇 제외), 스파이크 문서, Cross-wiki 트렌드, 논쟁 문서
  - **기사 생성** (`src/reporter/builder.py`): Claude Haiku (`claude-haiku-4-5-20251001`) 호출, 한국어 5섹션 브리핑
  - **Discord 발송** (`src/reporter/publisher.py`): 4개 Embed 카드 (헤드라인, 숫자 브리핑, 트렌딩, 논쟁)
