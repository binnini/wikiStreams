...
- [ ] **SLO 수립 로드맵** *(진행 중)*
  ...
  - [x] **1단계: 계측 보강 (Instrumentation Gap 해소)** *(2026-03-06 완료)*
    ...
    - [ ] S3 백업 구현 후 백업 완료 타임스탬프 로그 기록 → SLI-RC4 (RPO) 측정 가능화 *(S3 Datalake 도입 후)*
...
- [x] **`collector.py` 개선** *(2026-03-01, 2026-03-06 추가 완료)*
  ...
  - *(2026-03-06)* **배치 타임아웃 버그 수정**: 타임아웃 체크가 `if not sse.data:` 내부에 있어 Wikimedia heartbeat(SSE comment)가 `iter_sse()`에 노출되지 않아 타임아웃이 미발동. 루프 최상단으로 이동하여 모든 이벤트마다 체크.
  - *(2026-03-06)* **`Last-Event-ID` 재연결 지원**: 재연결 시 마지막 수신 이벤트 ID를 헤더로 전달 → 서버 백필로 이벤트 손실 최소화. 단, Wikimedia idle timeout(60초)이나 rate limit 상황에서는 무효.
  - *(2026-03-06)* **`_sse_received_total` 누적 카운터 추가**: 배치 처리 로그에 SSE 수신 누적 수 포함 (SLI-R5 계측)
  - **Wikimedia 60초 forced disconnect는 정상 동작**: 트래픽 없을 시 서버가 `:ok` heartbeat만 보내다 60초 idle 후 연결 종료. 재연결 루프는 의도한 동작이며 코드 버그 아님.
