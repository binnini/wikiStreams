...
- [ ] **리소스 이상 감지 알람 (Resource Anomaly Detection)**
  - `docker-stats-logger` (shell script) → Python `resource-monitor` 서비스로 교체
  - **메트릭 수집** (10초 간격, 전체 컨테이너):
    - `cpu_pct` — CPU 사용률 (%)
    - `mem_pct` — 메모리 사용률 (%)
    - `mem_mb` — 메모리 절댓값 (MB) — 점진적 leak 감지용
    - `block_io_mb` — 블록 I/O 델타 (MB/s) — ClickHouse merge/write 폭증 감지용
  - **Baseline** (`src/resource_monitor/baseline.py`): SQLite에 컨테이너별 × 시간대별(hour bucket 0–23) EMA + Welford online variance 저장. 재시작 후에도 학습 상태 유지.
  - **이상 감지** (`src/resource_monitor/detector.py`): z-score > 2.5 초과 시 이상으로 판단. 최소 샘플 미확보(학습 중) 기간은 감지 억제.
  - **감지 대상**: 기본값 `producer`, `clickhouse` — 설정(`MONITOR_TARGETS`)으로 컨테이너 추가 가능, 코드 수정 불필요
  - **Alert** (`src/resource_monitor/alerter.py`): Discord Embed 카드 발송. 같은 컨테이너·메트릭의 재발송 억제 시간 **1시간** (cooldown).
    - Embed 필드: 컨테이너명, 메트릭, 현재값, 이 시간대 EMA(평균), z-score
  - **Loki 로깅**: Anomaly 이벤트를 `level=warn msg="AnomalyDetected"` 구조화 로그로 출력 → Grafana Resources 대시보드에 이상 감지 이력 패널 추가 가능
  - **저장 구조**:
    - Raw 메트릭 → Loki (stdout, 기존 방식 유지)
    - Baseline 상태 (EMA·variance) → SQLite (`resource_monitor_baseline.db`)
    - Anomaly 이벤트 → Loki (warn 레벨 구조화 로그) + Discord
