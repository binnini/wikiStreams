...
| SLI-R5 | NFR-R5 | 파이프라인 완전성 | SSE 수신 이벤트 수 대비 ClickHouse 적재 이벤트 수 비율 (%) | — | **계측 불가** |
...
Producer가 SSE에서 수신한 이벤트 수를 현재 카운팅하지 않음. `collector.py`에 수신 이벤트 누적 카운터 로그 추가 후, ClickHouse `count()`와 비교하여 유실률 산출 가능.
...
| SLI-P6 | NFR-P6 | Kafka JMX Exporter 또는 `kafka-consumer-groups.sh` polling → Grafana 연동 |
| SLI-R5 | NFR-R5 | `collector.py`에 SSE 수신 이벤트 누적 카운터 로그 추가 |
| SLI-RC4 | NFR-RC4 | S3 백업 구현 후 백업 완료 타임스탬프 로그 기록 |
| SLI-RC5 | NFR-RC5 | 복원 runbook 실행 시 소요 시간 수동 기록 체계 마련 |
| SLI-CAP3 | NFR-CAP3 | `node-exporter` 추가 또는 `df -h` 기반 polling 스크립트로 호스트 디스크 모니터링 |
