...
| 분류 | 의미 |
|------|------|
| **자동 측정** | Grafana/Loki/QuestDB 쿼리로 상시 관측 가능 |
...
| ~~SLI-R2~~ | ~~NFR-R2~~ | ~~DLQ 재처리 시도 횟수~~ | ~~DLQ 컨슈머 재시도 로그 카운트~~ | ~~Loki~~ | ~~C2 제거로 폐기 (2026-03-07)~~ |
...
| SLI-R5 | NFR-R5 | 파이프라인 완전성 | SSE 수신 이벤트 수 대비 QuestDB 적재 이벤트 수 비율 (%) | Loki (`sse_received_total`) + QuestDB | 자동 측정 (부분) |
...
batch_size=3 sse_received_total=72
```
`sse_received_total`은 프로세스 재시작 시 초기화되므로, QuestDB `count(1)`와의 절대값 비교는 불가. 대신 배치 단위 유입량 대비 QuestDB 적재량의 상대적 추이로 유실 여부를 관찰. 정확한 비율 측정을 위한 대조 기준은 관측 기간 중 수립 예정.
...
| SLI-R5 | NFR-R5 | **부분 완료** (2026-03-06) | `sse_received_total` 추가, QuestDB 대조 기준 관측 중 |
| SLI-A2, P3, D1, D2, CAP1 | 각 NFR | **완료** (2026-03-10) | `pytest -m slo` SLO 테스트로 자동 검증 추가 |
| SLI-CAP3 | NFR-CAP3 | **완료** (2026-03-06) | `resource-monitor` `mem_mb` 합산 Loki 쿼리로 측정 가능 |
| SLI-RC4 | NFR-RC4 | **미완료** | S3 Datalake 구현 후 백업 완료 타임스탬프 로그 기록 |
