...
| **4단계** | Loki/Alloy 제거 + SLO 지표 QuestDB 이관 | **~947 MiB** | **46% ✅** | 2026-03-12 |

**총 절감: ~3,096 MiB (-77%)** — 4단계 완료 후 실측 (benchmark/loki-removal/ 기준)
...
| 컴포넌트 | 0단계 (기준) | 2단계 후 | 3단계 후 | 4단계 후 (현재) |
|---------|------------|---------|---------|----------------|
| 메시지 브로커 | kafka-kraft: **1,151 MiB** | redpanda: **382.8 MiB** | redpanda: **383.6 MiB** | redpanda: **288.6 MiB** |
| 데이터베이스 | clickhouse: **1,467 MiB** | clickhouse: **1,467 MiB** | questdb: **498.2 MiB** ¹ | questdb: **408.5 MiB** |
| DB Consumer | dlq-consumer: 27 MiB | (제거) | questdb-consumer: 17.7 MiB | questdb-consumer: **15.5 MiB** |
| producer | 45 MiB | 45 MiB | 50.5 MiB | **51.5 MiB** |
| reporter | 37 MiB | 37 MiB | 43.2 MiB | **5.4 MiB** ² |
| grafana | 130 MiB | 130 MiB | 131.2 MiB | **138.4 MiB** |
| loki | 156 MiB | 156 MiB | 172.5 MiB | **(제거)** |
| alloy | 102 MiB | 102 MiB | 106.0 MiB | **(제거)** |
| resource-monitor | 41 MiB | 41 MiB | 43.1 MiB | **38.2 MiB** |
| s3-exporter | — | — | — | **0.7 MiB** |
| **합계** | **~4,043 MiB** | **~3,214 MiB** | **~1,446 MiB** | **~947 MiB** |

> ¹ QuestDB: 전환 직후 381.5 MiB → 24h 데이터 누적 후 498.2 MiB (데이터 파일 증가)
> ² reporter: 4단계 측정 시 스케줄러 대기 중(비활성) 상태여서 낮게 측정됨 (실행 중 ~43 MiB)
...
### 4단계: Loki/Alloy 제거 + SLO 지표 QuestDB 이관

**완료**: 2026-03-12
**절감**: -499 MiB (컨테이너 합산 1,446 → 947 MiB), Swap -495 MiB

#### 변경 내용
- `loki`, `alloy` 컨테이너 및 이미지 제거 (디스크 778 MB 추가 절감)
- SLO 지표(`producer_slo_metrics`) QuestDB 직접 적재: `questdb-consumer`가 배치 처리 결과를 ILP로 기록
- Grafana SLO 대시보드 쿼리 QuestDB 전환 (P1/P2/P7/R1 time series 패널 포함)
- Resource Monitor 대시보드 QuestDB 전환

#### SLI 변화 (3단계 대비)

| SLI | 3단계 | 4단계 | 변화 |
|-----|-------|-------|------|
| 스택 메모리 | 1,446 MiB | **947 MiB** | **-499 MiB (-35%)** |
| Swap | ~700 MiB | **205 MiB** | **-495 MiB ✅** |
| CPU 합계 | ~57% (before) | **13.3%** | -57% |
| 컨테이너 수 | 10개 | **8개** | -2 |
| 배치 처리 avg | 0.835s | **0.723s** | -13% ✅ |
| 캐시 히트율 avg | — | **94.0%** | 신규 관측 ✅ |

#### 주요 관측

- **CPU 57% 감소의 실제 원인**: Loki/Alloy 자체 CPU는 1%에 불과. 로그 스트림이 생성하던 producer/redpanda/questdb의 처리 부하가 함께 사라진 효과.
- **QuestDB RSS 408 MiB (3단계 498 MiB 대비 감소)**: TTL 5d 적용 후 데이터 볼륨 수렴. SLO 지표 신규 테이블(`producer_slo_metrics`) 추가에도 전체 RSS 감소.
- **Swap 205 MiB**: OOM 위험 해소. t3.small에서 안정 운영 가능 수준.

#### 트레이드오프

| 항목 | 득 | 실 |
|------|----|----|
| 메모리 | -499 MiB (컨테이너) | QuestDB SLO 테이블 증가 +86.8 MiB (이관 비용) |
| 관찰성 | SLO 지표 QuestDB 단일화 (조회 통합) | 로그 집계 기능 완전 제거 (로그 검색 불가) |
| 복잡도 | 컨테이너 2개 감소 | questdb-consumer SLO 적재 코드 추가 관리 |
| 이미지 | 디스크 778 MB 절감 | — |
| 비용 | t3.small 안정 운영 확보 | — |

> **판정**: OOM 해소 목적 완전 달성. 로그 검색 기능 제거는 홈랩 수준에서 수용 가능. SLO 지표를 QuestDB로 일원화하여 Grafana 쿼리 복잡도도 감소.
...
4단계: █████████▍                                 947 MiB ( 46%) ✅  ← 안정화 후 실측
목표:  ████████████████████                     2,048 MiB (100% = t3.small)
```

### SLI 종합 (0단계 → 3단계)

| SLI | 0단계 | 3단계 (24h 실측) | 4단계 (안정화 실측) | 개선율 (0→4) |
|-----|-------|----------------|---------------------|-------------|
| 스택 메모리 | 4,043 MiB | 1,446 MiB | **947 MiB** | **-77%** |
| Swap 사용 | 측정 안 함 | 측정 안 함 | **205 MiB** | — |
| 배치 처리 p avg | 1.208s | 0.835s | **0.723s** | **-40%** |
| 캐시 히트율 | — | — | **94.0%** | — |
| DB 쿼리 p99 | 52ms | **9~16ms** | 9~16ms | **-69~83%** |
| 처리량 p5 | 1,166/min | 1,215/min | ~1,000/min ¹ | — |
| 데이터 lag | 7s | ~8s | **~9s** | 소폭 회귀 |
| 레이블 보강률 | 81.09% | 78.18% | **~79~88%** ⚠️ | 시간대별 편차 |
| 컨테이너 수 | 10+ | 10 | **8** | -2 |

> ¹ 4단계 처리량은 SLO v2 baseline 수집 중 (2026-03-10~). 목표 ≥800/min 유지 확인 중.
...
### 4단계 완료 — Loki/Alloy 제거 (2026-03-12)

**Option C (QuestDB 직접 메트릭) 채택**: Loki/Alloy 완전 제거 + SLO 지표(P1/P7) QuestDB 이관.
